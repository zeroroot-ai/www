...
#
# The Astro build bakes __APP_ORIGIN__ / __DOCS_ORIGIN__ into dist (see
# Dockerfile), so one image serves every environment: the chart sets
# APP_ORIGIN / DOCS_ORIGIN per environment and this script rewrites the
# static files once, before nginx starts (the stock entrypoint runs
# /docker-entrypoint.d/*.sh first). No env means prod — a bare
# `docker run` of this image serves the production links.
#
...
leftovers="$(grep -rl -e '__APP_ORIGIN__' -e '__DOCS_ORIGIN__' "$html_root" || true)"
if [ -n "$leftovers" ]; then
...
echo "40-substitute-origins: APP_ORIGIN=${APP_ORIGIN} DOCS_ORIGIN=${DOCS_ORIGIN}"
